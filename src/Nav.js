  ...
  return (
    <>
      <div className="bg-blue p-3 flex justify-between">
        <h1 className="font-chalk text-5xl text-white cursor-pointer" onClick={() => url.includes("development")?navigate(-1):navigate("/development")}>
          Blig
        </h1>
        {url.includes("scroll-page") && activeUser && activeUser.username && (
          <>
            <h1 className="my-auto mx-2 text-2xl text-white flex-1 text-center" style={{ fontSize: "min(5vw, 30px)" }}>
              {activeUser.username}
            </h1>
